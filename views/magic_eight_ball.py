...
from seo_keys import get_anthropic_key, get_apollo_key, get_semrush_key
...
def render() -> None:
    ...
    sources = []
    if apollo_key:
        sources.append("Apollo (companies & contacts)")
    if semrush_key:
        sources.append("Semrush (SEO & keywords)")
    if sources:
        st.caption(
            "I can pull from: " + ", ".join(sources) + ". Ask about a company's "
            "size/revenue/tech, its SEO traffic and keywords, contacts at a "
            "company, or companies matching criteria. Answering may consume "
            "Apollo/Semrush credits."
        )
    else:
        st.warning(
            "No data sources are connected (set `APOLLO_API_KEY` and/or "
            "`SEMRUSH_API_KEY`). I can only answer from general reasoning, not "
            "live data, until one is added."
        )
